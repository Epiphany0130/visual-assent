...
import com.guyuqi.backend.manager.FileManager;
import com.guyuqi.backend.manager.upload.FilePictureUpload;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (pictureId != null) {
            ...
            ThrowUtils.throwIf(!oldPicture.getUserId().equals(loginUser.getId()) && !userService.isAdmin(loginUser),
                    ErrorCode.NO_AUTH_ERROR, "仅本人和管理员可编辑");
            // 校验空间是否一致
            // 没传 spaceId，则复用原有的 spaceId
            if(spaceId == null) {
                if(oldPicture.getSpaceId() != null) {
                    spaceId = oldPicture.getSpaceId();
                }
            } else {
                // 传了 spaceId，必须和原图保持一致
                ThrowUtils.throwIf(ObjUtil.notEqual(spaceId, oldPicture.getSpaceId()), ErrorCode.PARAMS_ERROR, "空间 id 不一致");
            }
        } else {
            oldPicture = null;
        }
        ...
        transactionTemplate.execute(status -> {
            ...
            if (finalSpaceId != null) {
                boolean update;
                if (pictureId != null) {
                    // 更新图片：totalSize 加差值，totalCount 不变
                    update = spaceService.lambdaUpdate()
                            .eq(Space::getId, finalSpaceId)
                            .setSql("totalSize = totalSize - " + oldPicture.getPicSize() + " + " + picture.getPicSize())
                            .update();
                } else {
                    // 新增图片：totalSize 加新大小，totalCount + 1
                    update = spaceService.lambdaUpdate()
                            .eq(Space::getId, finalSpaceId)
    ...
    public QueryWrapper<Picture> getQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        String dominantColor = pictureQueryRequest.getDominantColor();
        ...
        queryWrapper.eq(StrUtil.isNotBlank(dominantColor), "dominantColor", dominantColor);
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(count > 30, ErrorCode.PARAMS_ERROR, "批量上传数量不能超过 30");
        ...
        for (Element imgElement : imgElementList) {
            ...
            if(StrUtil.isNotBlank(namePrefix)) {
                // 名称前缀_上传人_日期_序号
                pictureUploadRequest.setPicName(namePrefix + "_" + loginUser.getUserName() + "_" + DateUtil.formatDate(new Date()) + "_" + (uploadCount + 1));
            }
            try {
     ...
     * 按颜色搜索图片（支持公共图库和私有空间）
     *
     * @param spaceId 空间 id（可选，为空时查询公共图库）
    ...
    public List<PictureVO> searchPictureByColor(Long spaceId, String picColor, User loginUser) {
        ...
        // 3. 查询图片（必须有主色调）
        // 如果指定了空间，查询该空间的图片；否则查询公共图库（spaceId 为 null）
        List<Picture> pictureList;
        if (spaceId != null) {
            pictureList = this.lambdaQuery()
                    .eq(Picture::getSpaceId, spaceId)
                    .isNotNull(Picture::getDominantColorHex)
                    .list();
        } else {
            // 公共图库：spaceId 为 null 且已审核通过
            pictureList = this.lambdaQuery()
        ...
        List<Picture> sortedPictures = pictureList.stream()
                .sorted(Comparator.comparingDouble(picture -> {
                    // 提取图片主色调
                    String hexColor = picture.getDominantColorHex();
                    // 没有主色调的图片放到最后
